...
func extractResource(externalLabels, lset labels.Labels) (*monitoredres_pb.MonitoredResource, labels.Labels, error) {
	...
	if lset.Get(KeyProjectID) == "" {
		return nil, labels.EmptyLabels(), errors.Errorf("missing required resource field %q", KeyProjectID)
	}
	if lset.Get(KeyLocation) == "" {
		return nil, labels.EmptyLabels(), errors.Errorf("missing required resource field %q", KeyLocation)
	}
