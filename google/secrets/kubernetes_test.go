...
func TestProvider(t *testing.T) {
	...
	t.Run("ignore update to other secrets", func(t *testing.T) {
		s1 := &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Namespace: "ns1",
				Name:      "s1",
		...
		c := fake.NewSimpleClientset(s1.DeepCopy(), s2.DeepCopy())
		provider := newWatchProvider(t.Context(), log.NewNopLogger(), c)

		configS1 := &KubernetesSecretConfig{
			Namespace: "ns1",
			Name:      "s1",
			Key:       "k1",
		}

		providerSecretS1, err := provider.Add(configS1)
		require.NoError(t, err)
		requireFetchEquals(t.Context(), t, providerSecretS1, "initial-s1")

		// Update s2. Since the fake watch reactor sends all events in the namespace to the watcher,
		// the watcher for s1 will receive the update event for s2.
		s2Updated := s2.DeepCopy()
		s2Updated.Data["k1"] = []byte("updated-s2")
		_, err = c.CoreV1().Secrets("ns1").Update(t.Context(), s2Updated, metav1.UpdateOptions{})
		require.NoError(t, err)

		// Assert that fetching s1 still returns "initial-s1".
		requireFetchEquals(t.Context(), t, providerSecretS1, "initial-s1")

		// Now update s1 itself.
		s1Updated := s1.DeepCopy()
		s1Updated.Data["k1"] = []byte("updated-s1")
		_, err = c.CoreV1().Secrets("ns1").Update(t.Context(), s1Updated, metav1.UpdateOptions{})
		require.NoError(t, err)

		// Assert that fetching s1 now returns "updated-s1".
		requireFetchEquals(t.Context(), t, providerSecretS1, "updated-s1")

		provider.Remove(configS1)
		require.True(t, provider.isClean())
	})
