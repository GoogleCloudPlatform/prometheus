...
func describeMetric(name string, typ writev2.Metadata_MetricType) (
	...
	case writev2.Metadata_METRIC_TYPE_INFO, writev2.Metadata_METRIC_TYPE_STATESET:
		// TODO(bwplotka): Check if it's safe to do (new behaviour vs export pkg).
		suffix = gcmMetricSuffixGauge
		kind = metric_pb.MetricDescriptor_GAUGE
	case writev2.Metadata_METRIC_TYPE_UNSPECIFIED:
		// TODO(bwplotka): Consider adding support for /unknown cumulative + gauge metric flow
		// as https://github.com/GoogleCloudPlatform/prometheus/blob/ba13c6f51cee94d16c3e91da2e38dd00959a73bb/google/export/series_cache.go#L425-L437
		// However, it's unlikely we will have unspecified metrics at this point that are worth storing (e.g. they won't have CT likely).
		fallthrough
	default:
