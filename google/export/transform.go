...
func newSampleBuilder(c *seriesCache) *sampleBuilder {
	return &sampleBuilder{
		...
		touched: make([]uint64, 0, 128),
		results: make([]hashedSeries, 0, 128),
	}
}
...
func (b *sampleBuilder) next(metadata MetadataFunc, externalLabels labels.Labels, samples []record.RefSample, exemplars map[storage.SeriesRef]record.RefExemplar) ([]hashedSeries, []record.RefSample, error) {
	...
	if c := entry.protos.cumulative; c.proto != nil {
		if entry.metadata.Type == model.MetricTypeHistogram {
			var hs []hashedSeries
			var err error
			hs, tailSamples, err = b.buildDistributions(
			...
			resetTimestamp, val, ok = b.series.getResetAdjusted(storage.SeriesRef(sample.Ref), sample.T, sample.V)
			if ok {
				value = &monitoring_pb.TypedValue{
					Value: &monitoring_pb.TypedValue_DoubleValue{DoubleValue: val},
				}
...
// buildDistributions processes all series for a single histogram metric in the batch, and returns all completed distributions.
func (b *sampleBuilder) buildDistributions(
	...
	// The Prometheus/OpenMetrics exposition format requires all histogram series for a single distribution
