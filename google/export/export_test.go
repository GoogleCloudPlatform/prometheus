...
func TestDisabledExporter(t *testing.T) {
	...
	// ApplyConfig with configuration changes that would normally trigger client recreation
	// (e.g. credentials file or compression). Since the exporter is disabled, it should
	// bypass validation/recreation and return immediately with no error.
	cfg := &config.Config{}
	cfg.GoogleCloud.Export.CredentialsFile = "new-credentials-file-that-does-not-exist.json"
	cfg.GoogleCloud.Export.Compression = CompressionGZIP

	if err := e.ApplyConfig(cfg); err != nil {
		t.Fatalf("ApplyConfig on disabled exporter failed: %s", err)
	}

	// Verify that the exporter fields were not updated (since it was a no-op).
	if e.opts.CredentialsFile != "" {
		t.Errorf("Expected empty CredentialsFile, got %s", e.opts.CredentialsFile)
	}
	if e.opts.Compression != CompressionNone {
		t.Errorf("Expected CompressionNone, got %s", e.opts.Compression)
	}
