...
function mergeEdges(edges) {
    ...
    let eof = edges.filter(e => e.from == 65535 /* End */);
    if (eof.length) {
        let found = [];
        for (let edge of eof)
            for (let target of edge.target.closure())
                if (!found.includes(target))
                    found.push(target);
        if (found.length)
            result.push(new MergedEdge(65535 /* End */, 65535 /* End */, found));
    }
    return result;
}
    ...
    getParserFile() {
        ...
        let specialized = rawSpecialized.map(v => {
            if (v instanceof ExternalSpecializer) {
                ...
                return `{term: ${v.term.id}, get: (value, stack) => (${name}(value, stack) << 1)${v.ast.type == "extend" ? ` | ${1 /* Extend */}` : ''}, external: ${name}${v.ast.type == "extend" ? ', extend: true' : ''}}`;
            }
            else {
